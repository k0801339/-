...
void UBikeSystemIMP::Rent(std::string classType, std::string station)
{
	...
	if(!heap.isEmpty()){
		UBike *u = heap.removeUBikePtr(1);	//change'1' if the head of heap is '0'
		ubStations.at(station).at("Rented").addUBikePtr(u);	//in function change heapindex
		u->isRented = true;
	}
}
void UBikeSystemIMP::Return(std::string station, std::string license, int returnMile)
{
	...
	if(u!=nullptr&&u->isRented){
		...
		u->isRented = false;
		UBike *ptr = ubStations.at(u->station).at("Rented").removeUBikePtr(u->heapIndex);
		//ptr->station = station;
